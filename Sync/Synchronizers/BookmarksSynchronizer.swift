...
class ThreeWayTreeMerger {
    ...
    var remoteValueQueue: [BookmarkTreeNode] = []     // Need value reconciling.
    ...
    func merge() -> Deferred<Maybe<BookmarksMergeResult>> {
        ...
        do {
            repeat {
                if let item = self.remoteQueue.popLast() {
